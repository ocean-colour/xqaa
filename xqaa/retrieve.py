...
@dataclass
class XQAARetrieval:
    """ Container for the IOPs retrieved by :func:`iops_from_Rrs`.

    Generated by JXP and Claude.

    Attributes:
        anw (np.ndarray): Non-water absorption coefficient [1/m], per wavelength.
        bbp (np.ndarray): Particulate (non-water) backscattering coefficient
            [1/m], per wavelength.
        corr_bbp (np.ndarray or float): The backscatter correction that was
            applied when solving for ``anw`` (see ``XQAAParams.bbp_corr``).
        avg_bbp (float): Mean particulate backscatter over the
            ``[bbmin, bbmax]`` window used to build the correction.
    """
    anw: np.ndarray
    bbp: np.ndarray
    corr_bbp: object
...
def iops_from_Rrs(wave: np.ndarray, Rrs: np.ndarray,
    ...
    Returns:
        XQAARetrieval: The retrieved ``anw`` and ``bbp`` spectra along with the
            ``corr_bbp`` correction and the ``avg_bbp`` window mean.
    """
    ...
    return XQAARetrieval(anw=anw, bbp=bbp, corr_bbp=corr_bbp, avg_bbp=avg_bbp)
